...
This text & source code is available under the MIT license from [Github](https://github.com/hoehleatsu/When2Stop). Furthermore, it is also available as a [RPubs](http://rpubs.com/hoehle/179204).
...
K_days <- head(cumsum(table(cut(K, breaks=c(0:(max(t)),Inf)))),n=-1)
K_days
...
stoptime_at_time <- function(time=NULL,mu.hat=NULL,testProcess, plot=TRUE) {
  if (!is.null(time)) {
...
stopTime <- stoptime_at_time(time=max(t),mu.hat=mu.hat,testProcess=testProcess,plot=TRUE)
