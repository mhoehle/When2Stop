...
This is a small note rediscovering a gem published by S. R. Dalal and C. L. Mallows about
...
shipping. The problem is translated into a sequential decision problem, where an optimal stopping rule has to be found in order minimize expected loss.
We sketch the main result of their article and apply their stopping rule to an example
using R code.

This text is available under the MIT license from [Github](https://github.com/hoehleatsu/When2Stop) and as a [RPubs](http://rpubs.com/hoehle/179204).
...
mu <- 0.02
n <- 1250
#Simulate K based onthe inter arrival times of the exponential distribution.
#See
#http://www.math.kth.se/matstat/gru/sf2955/exponorderstats.pdf
K <- cumsum(rexp(n,rate=(n+1-1:n)*mu))
...
In the simple model with exponential discovery times having rate $\mu$, the stopping rule  stated as equation (4.6) of Dalal and Mallows (1988) is to stop as soon as the number, $k$, of bugs found at time $t$ is such that:
$$
...
In the above, the quantity $c/f$ measures the amount saved by finding a bug (and hence fixing it before release) measured in units of testing days. As an example: if $c/f=0.2 \Leftrightarrow f/c=5$ then the gain in detecting a bug before release corresponds to 0.2 testing days.
...
ll <- function(theta, testProcess, tC=max(testProcess$t)) {
  ...
  #CDF function taking the truncation into account
  CDF <- function(x) pexp(x,rate=mu)/pexp(tC,rate=mu)
  sum(DeltaK * log(CDF(1:(max(testProcess$t)+1)) - CDF(testProcess$t)))
}
...
#
# Paramaters:
#  time   - timepoint of the testing period
...
stoptime_at_time <- function(time=NULL,mu.hat=NULL,plot=TRUE) {
  if (!is.null(time)) {
...
stopTime <- stoptime_at_time(time=max(t),mu.hat=mu.hat,plot=TRUE)
stopTime

```

The optimal stopping time in the example in the case of $f/c=5$ is to stop the testing after `r stopTime$stopTimefc5$t` testing days. An estimate of the expected number of remaining bugs at this stopping time would be `r sprintf("%.1f",5/mu.hat)`, which appears to agree quite well with the empirical data!
...
saveGIF(sapply(t_grid,stoptime_at_time))
```
